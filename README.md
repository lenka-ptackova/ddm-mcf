...
When the function `main()` in the script **main.py** is called, the examples from figures in the article are processed. The given meshes are flown by the mean curvature flow with the given methods and resulting meshes are. Information about the mean curvature is printed and t, information about the mean curvatures are printed on the screen, and the smoothed meshes are exported to .obj files in the folder `meshes/output`. These meshes then can be visualized using any .obj file viewer such as *Meshlab*.

**Important**: Throughout the code, the precision is set to **np.float32** and **np.int32**. You might want to increase the precision, i.e., set parameters `float_precis` and `int_precis` to np.float64 and np.float64, resp. Concretely, the integer precision must be increased if very large meshes are to be processes.
